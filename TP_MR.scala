...
object TP_MR {
  ...
  def lastA[T](l: List[T]): Option[T] =
    l match {
      case Nil => None
      case h :: Nil => Some(h)
      case h :: tail => lastA(tail)
    }
  ...
  println(lastB(List(1, 1, 2, 3, 5, 8)))
  ...
  def reverseA[T](list: List[T]): List[T] = list.foldLeft(List[T]()) {
    //默认起始为0，后运行结果添加到list中，类似数据结构list的单链表？
    (r,h)=>h :: r
  }

  //TODO define reverseB using method of the List collection
  def reverseB[T](list: List[T]): List[T] = list.reverse
  println(reverseB(List(1, 1, 2, 3, 5, 8)))
  ...
  def salarySum(employees: List[Employee]): Double = {
    ...
    val report=(netPay foldLeft(0.0)){
      case((totalNet),(e,net))=>
        (totalNet+net)
    }
    report
  ...
  //reduceOption{(x1,x2)=>x1+x2}--如果list是空则无法reduce
  ...
  def average(it : Iterator[Double]): Option[Double] = {
    var n=0
    var sum=0
    if (it.isEmpty){
      None
